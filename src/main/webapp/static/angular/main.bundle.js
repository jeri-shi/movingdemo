...
var AdduserComponent = (function () {
    ...
    AdduserComponent.prototype.addUser = function () {
        ...
        this.userService.addUser(this.newUser).then(function (user) {
            ...
            var fileBrower = _this.fileInput.nativeElement;
            if (fileBrower.files && fileBrower.files[0]) {
                var formData = new FormData();
                formData.append("file", fileBrower.files[0]);
                _this.userService.uploadImage(user.id, formData).then(function (response) {
                    console.log(response);
                    //after upload image, display it
...
var EdituserComponent = (function () {
    ...
    EdituserComponent.prototype.ngOnInit = function () {
        ...
        // this.fillInForm();
        var _this = this;
        this.createForm(this.route.snapshot.paramMap.get('id'));
    ...
    EdituserComponent.prototype.createForm = function (id) {
    ...
    EdituserComponent.prototype.updateUser = function () {
        ...
        this.userService.updateUser(this.editUser).then(function (user) {
            ...
            _this.serverError = undefined;
            _this.fillInForm();
            //if photo file is specified, then upload it to replace server's photo
            var fileBrower = _this.fileInput.nativeElement;
            if (fileBrower.files && fileBrower.files[0]) {
                var formData = new FormData();
                formData.append("file", fileBrower.files[0]);
...
var UserService = (function () {
    ...
    UserService.prototype.uploadImage = function (id, formData) {
        ...
        return this.http.post(this.userUrl + "/" + id + "/photo", formData, options)
            .toPromise()
    ...
    UserService.prototype.setCurrentPage = function (page) {
    ...
    UserService.prototype.getUserListCount = function (param) {
        console.log("UserService.getUserList()..." + param);
        return this.http.post(this.usersListCountUrl, JSON.stringify(param), { headers: this.headers }).toPromise()
            .then(function (response) {
            console.log("UserService.getUserListCount() -> response = " + response);
            return response.json();
        })
            .catch(this.handleError);
    };
    UserService.prototype.getUserList = function (param) {
        ...
        return this.http.post(this.usersListUrl, JSON.stringify(param), { headers: this.headers }).toPromise()
            .then(function (response) {
            console.log("UserService.getUserList() -> response = " + response);
            return response.json();
        })
            .catch(this.handleError);
        // return [
...
var UserdetailComponent = (function () {
    ...
    UserdetailComponent.prototype.createForm = function (id) {
        this.userForm = this.fb.group({
            id: id,
            company: '',
            username: '',
            role_user: [{ value: false, disabled: true }],
            role_admin: [{ value: false, disabled: true }]
        });
    };
